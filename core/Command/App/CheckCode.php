	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		if (!$input->getOption('skip-validate-info')) {
			...
			$infoChecker->listen('InfoChecker', 'missingRequirement', function ($minMax) use ($output) {
				$output->writeln("<comment>$minMax server version requirement missing (will be an error in version 11 and later)</comment>");
			});

			$infoChecker->listen('InfoChecker', 'duplicateRequirement', function ($minMax) use ($output) {
				$output->writeln("<error>Duplicate $minMax server version requirement found</error>");
			});
